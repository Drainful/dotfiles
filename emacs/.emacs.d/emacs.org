...
#+BEGIN_SRC emacs-lisp
  ...
  Amount of increase is specified by `pulseaudio-control-volume-step'."
    ...
    (let* ((volume-step (progn
           ...
           (percentage-right (string-to-number (match-string 6 volumes-current) "0")))
...
#+BEGIN_SRC emacs-lisp
      ...
      (with-current-buffer buffer
        (when (eq major-mode 'erc-mode)
          (setq right-margin-width (or width
                                       (max 10 (- (+ (window-width (get-buffer-window))
                                                     (cdr (window-margins (get-buffer-window))))
                                                  102))))
  ...
  ;; (defun me/erc-margin-remove (oldfun window &rest args)
  ;;   (if (eq major-mode 'erc-mode)
  ;;       (let ((old-width right-margin-width))
  ;;         (me/erc-margin-adjust (window-buffer window) 0)
  ;;         (apply oldfun window args)
  ;;         (me/erc-margin-adjust (window-buffer window)))
  ;;     (apply oldfun window args)))

  ;; (advice-add 'split-window
  ;;             :around
  ;;             #'me/erc-margin-remove)
  ...
  (defun me/erc-setup ()
    ...
    ;;           #'me/erc-margin-adjust-delayed nil t)
    )
