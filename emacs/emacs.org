...
#+BEGIN_SRC emacs-lisp
  (defun nixconf ()
    (interactive)
    (or (find-file "~/nixos-config/modules/core.nix")
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;       '(:eval (unless (string= (cdr (assq 66 fancy-battery-last-status)) "Charging")
...
#+BEGIN_SRC emacs-lisp
  (use-package helm
    ...
    (setq helm-display-buffer-default-height 34)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; I'll stick with default for noww
...
#+BEGIN_SRC emacs-lisp
  (use-package skeletor
    ...
    ;; Extending built-in project types
...
*** Keybindings
#+BEGIN_SRC emacs-lisp
  (general-define-key :states 'normal :mode 'sly-mode-map
  ...
  (use-package exwm
    ...
    (setq exwm-input-global-keys
          `(;; Bind "s-r" to exit char-mode and fullscreen mode.
            ...
            ([?\s-2] . eshell-at-or-switch)
            ([?\s-3] . exlaunch/alacritty)
            ([?\s--] . helm-exwm)
...
#+BEGIN_SRC emacs-lisp
  ...
  (exlaunch-shortcuts
   ...
   (multimc :in-workspace "minecraft")
...
#+BEGIN_SRC emacs-lisp
  (use-package desktop-environment
    ...
    (general-define-key ; :keymaps 'dekstop-environment-mode-map
                        "<C-XF86MonBrightnessUp>" #'desktop-environment-brightness-increment-very-slowly
                        "<C-XF86MonBrightnessDown>" #'desktop-environment-brightness-decrement-very-slowly))
#+END_SRC
...
*** Filtering
Functions to select exwm buffers. 
#+BEGIN_SRC emacs-lisp
  (require 'helm-exwm)

  (defun exwm-buffers-of-class (class)
    (let ((filter (lambda ()
                    (string= (downcase (or exwm-class-name ""))
                             (downcase class)))))
      (helm-exwm-candidates filter)))

  (provide 'exwm-filtering)
#+END_SRC
*** Qutebrowser
#+BEGIN_SRC emacs-lisp
  (require 'exwm-filtering)

  (defun qute-read-buffer-or-url ()
    (let ((pointless " - qutebrowser"))
      (concat (completing-read "URL: "
                               (mapcar (lambda (buffer-name)
                                         (substring buffer-name 0 (- (length buffer-name)
                                                                     (length pointless))))
                                       (exwm-buffers-of-class "qutebrowser")))
              pointless)))

  (defun qute-browse (url &rest args)
    (interactive (list (qute-read-buffer-or-url)))
    (if (get-buffer url)
