...
#+BEGIN_SRC emacs-lisp
  (defun nixconf () (interactive) (find-file "/sudo::/etc/nixos/configuration.nix"))
#+END_SRC
...
per-file basis. Direnv integrates with Nix as well.
#+BEGIN_SRC emacs-lisp
  (use-package direnv
    :delight
            ...
            (if (not (get-process process-name))
                (progn
                  ...
                  (start-process-shell-command
                   ...
                   "lorri watch")
                  (message (concat "Lorri watching " (projectile-project-name))))
              (error (concat "Lorri is already watching " (projectile-project-name))))
          (error (concat "There is no shell.nix for " (projectile-project-name))))))
