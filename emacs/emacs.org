...
#+BEGIN_SRC emacs-lisp
  ...
  (create-multi-macro simple-text-objects
...
#+BEGIN_SRC emacs-lisp
  ...
  (create-multi-macro make-conditional-key-translations
  ...
  (create-multi-macro inoremap inoremap-single 2)

  (defmacro noremap-single (from to)
    `(make-conditional-key-translation ,from ,to (lambda nil t)))
  (create-multi-macro noremap noremap-single 2)
#+END_SRC
** All modes
#+BEGIN_SRC emacs-lisp
  ;; (noremap "9" "("
...
(create-multi-macro define-shell-functions define-shell-function 2)
