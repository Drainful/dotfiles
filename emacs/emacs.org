  ...
  (use-package exwm
    ...
    (setq exwm-input-global-keys
          `(;; Bind "s-r" to exit char-mode and fullscreen mode.
            ...
            ;; Bind "s-&" to launch exteral through helm
            ...
            ;;workspaces
            ([?\s-m] . exwm-namedworkspace-make)
            ([?\s-d] . exwm-namedworkspace-delete)

            ([?\s-n] . exwm-namedworkspace-next)
            ([?\s-p] . exwm-namedworkspace-previous)

            ([s-tab] . exwm-namedworkspace-switch)
...
**** Implementation
#+BEGIN_SRC emacs-lisp
  (defconst launcher-prefix "launcher/")
  (defvar launcher-shortcut-functions nil
    "List of functions representing launcher shortcuts.")


  (defun launcher--get-exwm-buffer (class)
  ...
  (cl-defmacro launcher-shortcut (name &key shell-command args
                                       (switch-to t)
                                       exwm-class
                                       other-window
                                       before
  ...
  exist."
    (let* ((name-string (symbol-name name))
           ...
           (function-symbol (intern (concat launcher-prefix name-string)))
      ...
      `(progn
         (defun ,function-symbol ()
           ...
           ,(if (require 'exwm nil t)
                `(progn
                   ,(when (and t ;(require 'exwm-named-workspaces nil t)
                               in-named-workspace)
                   ...
                   (if ,(when switch-to `(get-process ,name-string))
                       (,(if other-window
                           ...
                           'switch-to-buffer)
                        (launcher--get-exwm-buffer ,exwm-class))
                     (progn
         ...
         (add-to-list 'launcher-shortcut-functions ',function-symbol))))


  (define-multi-macro-clauses launcher-shortcuts launcher-shortcut
    "Define functions to launch programs. ")

  (defun read-launcher ()
    (completing-read "Launch: " launcher-shortcut-functions))

  (defun launch ()
    (interactive)
    (funcall (symbol-function (intern (read-launcher)))))
#+END_SRC
**** Usage
#+BEGIN_SRC emacs-lisp
  (launcher-shortcuts
   (firefox :switch-to nil)
   krita
   gimp
   (spotify :args "--force-device-scale-factor=2"
            :other-window t)
   (wifi :shell-command "alacritty"
         :args "-e nmtui")
